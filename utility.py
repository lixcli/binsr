...
class checkpoint():
    def __init__(self, args):
        ...
        if not args.load:
            if not args.save:
                args.save = now
            self.dir = os.path.join('..', 'binsr_experiment', args.save)
        else:
            self.dir = os.path.join('..', 'binsr_experiment', args.load)
...
def make_schedule(args):
    if args.schedule == 'MultiStepLR':
        return lrs.MultiStepLR
    elif args.schedule == 'CosineAnnealingLR':
...
def make_optimizer(args, target):
    ...
    scheduler_class = make_schedule(args)
